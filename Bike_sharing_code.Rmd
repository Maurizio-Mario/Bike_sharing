...
Such an hypothesis is supported by a simple observation of the frequency of the rents according to the weather conditions. 
...
At the same time, could be interesting an observation of the number of casual rental according to the day of the week.
...
We can see that there is a relevant peak if rents during the week end.

```{r data exploration 5, fig.width=10}
with(all_data, plot(Weekday2, Casual.renter, main = "Casual renters by weekday"))
```
